...
const architectureLayers = [
    ...
    { name: "Web Interface", tech: "Next.js 14 + React", color: "#10b981" },
...
export default function MarketMindHero() {
    ...
    // live metrics hook
    const { metrics: liveMetrics, loading } = useLiveMetrics();

    const latencyText = liveMetrics ? `${liveMetrics.latencyP95Ms.toFixed(1)} ms p95` : "–";
    const throughputText = liveMetrics ? `${liveMetrics.throughputPerSec.toLocaleString()}/s` : "–";
    const winRateText = liveMetrics ? `${(liveMetrics.winRate * 100).toFixed(1)}%` : "–";
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setActiveMetric((prev) => (prev + 1) % staticMetrics.length);
        }, 3000);
        ...
        <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden">
            ...
            <section aria-labelledby="hero-heading" className="relative">
                <div className="max-w-7xl mx-auto px-6 pt-20 pb-16">
                    <div
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-8 transition-opacity duration-1000 ${mounted ? "opacity-100" : "opacity-0"
                            }`}
                    >
                        <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
                        <span className="text-sm text-emerald-400 font-medium">
                            Hybrid GPU-Accelerated HFT Engine (C++ · Python · TensorRT)
                        </span>
                    </div>
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 items-center">
                        <div>
                            <h1
                                id="hero-heading"
                                className={`text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 leading-tight transition-all duration-1000 ${mounted ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                                    }`}
                            >
                                Trade at the
                            ...
                            <p
                                className={`text-lg sm:text-xl text-slate-300 mb-4 leading-relaxed transition-all duration-1000 delay-200 ${mounted ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                                    }`}
                            >
                                MarketMind is an advanced algorithmic trading platform that couples
                                hybrid Transformer models with an ultra-low-latency C++ runtime. Deploy
                                ONNX/TensorRT engines and execute strategies with strict SLOs for
                                latency, risk, and capacity.
                            </p>

                            <p
                                className={`text-sm text-slate-400 mb-8 transition-all duration-1000 delay-250 ${mounted ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                                    }`}
                            >
                                Built for quant funds, HFT desks, and advanced retail algos that need
                                real performance—not spreadsheets.
                            </p>

                            <div
                                className={`flex flex-wrap gap-4 mb-4 transition-all duration-1000 delay-300 ${mounted ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                                    }`}
                            >
                                <a
                                    href="/quickstart"
                            ...
                            >
                                {staticMetrics.map((metric, idx) => {
                                    ...
                                    const displayValue =
                                        idx === 0 ? latencyText : idx === 1 ? throughputText : winRateText;
                                    return (
                                        <button
                                            key={metric.label}
                                            type="button"
                                            onClick={() => handleMetricClick(idx)}
                                            className={`text-left rounded-xl p-4 border transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${isActive
                        ...
                        <div
                            className={`relative transition-all duration-1000 delay-500 ${mounted ? "translate-x-0 opacity-100" : "translate-x-8 opacity-0"
                                }`}
                        >
            ...
            <section className="relative max-w-7xl mx-auto px-6 py-24">
                <div className="text-center mb-16">
                    ...
                    <p className="text-lg sm:text-xl text-slate-400">
                        Measured inference latency across production workloads
                    </p>
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                    <div className="space-y-3">
                        {architectureLayers.map((layer, idx) => (
                            <div
                                key={layer.name}
                                className={`bg-slate-900/30 backdrop-blur-sm border rounded-xl p-5 transition-all duration-300 ${activeLayer === idx
                    ...
                    <div className="grid md:grid-cols-4 gap-8 mb-8">
                        ...
                        <div>
                            ...
                            <ul className="space-y-2 text-sm text-slate-400">
                                ...
                                <li>
                                    <a href="/integrations" className="hover:text-emerald-400 transition">
                                        Integrations
                                    </a>
                                </li>
                            </ul>
                        ...
                        <div>
                            ...
                            <ul className="space-y-2 text-sm text-slate-400">
                                <li>
                                    <a href="/docs" className="hover:text-emerald-400 transition">
                                        Documentation
                                    </a>
                                </li>
                                <li>
                                    <a href="/api" className="hover:text-emerald-400 transition">
                                        API Reference
                                    </a>
                                </li>
                                <li>
